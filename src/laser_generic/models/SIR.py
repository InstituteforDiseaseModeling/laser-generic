        ...
        for node in range(self.model.nodes.count):
            ...
            if (ndeaths := self.model.deaths[tick, node]) > 0:
                ...
                if ndeaths > 0:
                    ...
                    i_recovered = i_to_remove[np.nonzero(self.model.people.state[i_to_remove] == State.RECOVERED.value)[0]]
                    nrec_deaths = len(i_recovered)

                    self.model.people.state[i_to_remove] = State.DECEASED.value

                    # Update patch counts
                    assert nsus_deaths + ninf_deaths + nrec_deaths == ndeaths, "Death state count mismatch"
                    # state(t+1) = state(t) + ∆state(t)
                    self.model.nodes.S[tick + 1, node] -= nsus_deaths
                    self.model.nodes.I[tick + 1, node] -= ninf_deaths
                    self.model.nodes.R[tick + 1, node] -= nrec_deaths
                    # Record today's ∆
